...
class SimilarityCompare:
    def __init__(self):
        ...
        self.folder_path = os.path.join("/home/yzc/project/navigation/visualnav-transformer/deployment/topomaps/rgbd/sqz_27_7")
        self.match_num = 0
        self.counter = 0
    ...
    def determine_destination(self):
        ...
        def judge_monotonicity(self):
            if (len(self.match_diff) < 7 or less_than_all(self.match_counter[-7: -1], 100)):
                return "invalid"
            else:
                if greater_than_all(self.match_diff[-7: -1], 0):
                    return "increasing"
                elif less_than_all(self.match_diff[-7: -1], 0):
                    return "decreasing"
                else:
                    return "nonmonotonic"
