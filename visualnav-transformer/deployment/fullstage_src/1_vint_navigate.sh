...
tmux send-keys "conda activate vint_deployment" Enter
tmux send-keys "python3 navigate_visualize.py --model vint --dir $traj_name" Enter

# Run the pd_controller.py script in the fourth pane
tmux select-pane -t 2
tmux send-keys "conda activate vint_deployment" Enter
tmux send-keys "bash ./2_find_next_pose.sh --model vint --dir $traj_name" Enter
