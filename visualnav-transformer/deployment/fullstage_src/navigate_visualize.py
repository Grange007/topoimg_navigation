...
def main(args: argparse.Namespace):
    ...
    closest_node_pub = rospy.Publisher(
        "/closest_img", Image, queue_size=1
    )
    bridge = CvBridge()
    ...
    log_dir = f"../topomaps/logs"
    log_file = os.path.join(log_dir, f"{args.dir}_{current_time}.txt")
        ...
        while not rospy.is_shutdown():
            ...
            if len(context_queue) > model_params["context_size"]:
                ...
                if model_params["model_type"] == "nomad":
                    ...
                    closest_img = topomap[closest_node]
                    print(closest_img)
                    closest_img_msg = bridge.cv2_to_imgmsg(np.array(closest_img), encoding="rgb8")
                    closest_node_pub.publish(closest_img_msg)
                    ...
                    f.write(f"closest_node: {closest_node}\n")
                    f.write(f"distances: {distances.tolist()}\n")
                    closest_img = topomap[closest_node]
                    closest_img_msg = bridge.cv2_to_imgmsg(np.array(closest_img), encoding="rgb8")
                    closest_node_pub.publish(closest_img_msg)
