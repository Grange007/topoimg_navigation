...
def main(args: argparse.Namespace):
    ...
    rospy.init_node("CREATE_TOPOMAP_"+IMAGE_TOPIC[1:], anonymous=False)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=f"Code to generate topomaps"
    )
    parser.add_argument(
        "--dir",
        "-d",
    ...
    parser.add_argument(
        ...
        default=1.,
        type=float,
        help=f"time between images sampled from topic (default: 3.0)",
    )
    parser.add_argument(
        "--topic",
        "-p",
