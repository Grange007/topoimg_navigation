...
# command
First, run `start_connect.sh` in `B1_SDK` folder to connect to some usbs. 

cd to langzhe's new src folder.
roscore
bash _record_bag.sh sqzxxx
tmux kill-server
bash create_topomap.sh sqzxxx xxxx.bag

Then here we should close the roscore and launch the b1.launch 
bash _navigate.sh "--model vint --dir sqzxxxx"
